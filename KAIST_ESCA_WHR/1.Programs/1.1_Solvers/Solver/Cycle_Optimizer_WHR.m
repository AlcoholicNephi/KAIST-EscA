function [P, Junc, info] = Cycle_Optimizer_WHR(Condition, Component, Mechanics, iParameters, mu_0)
...
while(1)
    ...
    disp(['Cycle Efficiency (%) = ', num2str(info.Net_work / info.Q_in)]);
...
%             end
        ...
        if(abs(dg / g0) < Condition.Error_bound * 100)
            if(old_g > g0)
                Parameters_now = old_Parameters;
            end
            break;
