function [Junc, info] = Steady_Solver_WHR(Condition, Component, Mechanics)
            ...
            case 'WHR'
                for j=1:2
                    if(Junc(Component(i).inJunc(j)).P ~= -1)
                        if(Mechanics.WHR(Component(i).index).dP(j) > 0)
                            Junc(Component(i).outJunc(j)).P = Junc(Component(i).inJunc(j)).P - Mechanics.WHR(Component(i).index).dP(j);
                        else
                            Junc(Component(i).outJunc(j)).P = Junc(Component(i).inJunc(j)).P * (1 + Mechanics.WHR(Component(i).index).dP(j));
                        end
                    elseif(Junc(Component(i).outJunc(j)).P ~= -1)
                        if(Mechanics.WHR(Component(i).index).dP(j) > 0)
                            Junc(Component(i).inJunc(j)).P = Junc(Component(i).outJunc(j)).P + Mechanics.WHR(Component(i).index).dP(j);
                        else
                            Junc(Component(i).inJunc(j)).P = Junc(Component(i).outJunc(j)).P / (1 + Mechanics.WHR(Component(i).index).dP(j));
                        end
                    end
                end
                %%%
...
for i=1:size(Component, 2)
    ...
    elseif(strcmp(Component(i).type, 'WHR'))
        dH1 = refpropm('H', 'T', Junc(Component(i).outJunc(1)).T, 'P', Junc(Component(i).outJunc(1)).P, Condition.Fluid) ...
            - refpropm('H', 'T', Junc(Component(i).inJunc(1)).T, 'P', Junc(Component(i).inJunc(1)).P, Condition.Fluid);
        dH = dH + dH1 * Junc(Component(i).inJunc(1)).m;
    end
end

Q_in = dH*Condition.m_flow_rate;
...
info.Eff = info.Net_work / Q_in;
info.m_flow_rate = Condition.m_flow_rate;
info.WorkT = WorkT;
info.WorkC = WorkC;
